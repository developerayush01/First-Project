...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete'])) {
    if (isset($_POST['roll_no'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM class_9 WHERE Roll_no = ?");
        $stmt->bind_param("s", $roll_no);

         if ($stmt->execute() === TRUE) {
            // Reorder the roll numbers
            $conn->query("SET @count = 0");
            $conn->query("UPDATE class_9 SET Roll_no = @count:= @count + 1");
            $conn->query("ALTER TABLE class_9 AUTO_INCREMENT = 1");

            echo "<script>alert('Student record deleted and roll numbers updated.');
                  window.location.href = 'class9.php';</script>";
            exit();
        } else {
...
<body>
  <div class="navbar">
        ...
        <a href="add_student9.php">Add New Student</a>
    ...
    <div class="container">
        ...
        <div class="table-container">
            <table border="1">
                ...
                $sql = "SELECT Name, Roll_no, Address, Father_name, Contact_no, Total_fees, Cleared_Fees, (Total_fees - Cleared_Fees) AS Remaining_Fees FROM class_9";
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>
                                <form method='GET' action='update_form9.php' style='display:inline;'>
