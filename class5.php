...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete'])) {
    if (isset($_POST['roll_no'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM class_5 WHERE Roll_no = ?");
        $stmt->bind_param("s", $roll_no);

          if ($stmt->execute() === TRUE) {
            // Reorder the roll numbers
            $conn->query("SET @count = 0");
            $conn->query("UPDATE class_5 SET Roll_no = @count:= @count + 1");
            $conn->query("ALTER TABLE class_5 AUTO_INCREMENT = 1");
            // Perform JavaScript redirection
            echo "<script>alert('Student record deleted successfully.');
                  window.location.href = 'class5.php';</script>";
            exit();
        } else {
...
$stmt->bind_param("i", $roll_no_to_delete);
$stmt->execute();
$stmt->close();

// Roll number to update and new roll number
$current_roll_no = 102;
$new_roll_no = 101;

// Update the roll number of another student
$update_sql = "UPDATE students SET roll_no = ? WHERE roll_no = ?";
$stmt = $conn->prepare($update_sql);
$stmt->bind_param("ii", $new_roll_no, $current_roll_no);
$stmt->execute();
$stmt->close();
...
<body>
    <div class="navbar">
        ...
        <a href="add_student5.php">Add New Student</a>
    ...
    <div class="container">
        ...
        <div class="table-container">
            <table border="1">
                ...
                $sql = "SELECT Name, Roll_no, Address, Father_name, Contact_no, Total_fees, Cleared_Fees, (Total_fees - Cleared_Fees) AS Remaining_Fees FROM class_5";
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>
                                <form method='GET' action='update_form5.php' style='display:inline;'>
