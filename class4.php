...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete'])) {
    if (isset($_POST['roll_no'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM class_4 WHERE Roll_no = ?");
        $stmt->bind_param("s", $roll_no);

        // Execute the statement
        // Prepare the SQL statement
        $stmt = $conn->prepare("DELETE FROM class_8 WHERE Roll_no = ?");
        $stmt->bind_param("s", $roll_no);

          if ($stmt->execute() === TRUE) {
            // Reorder the roll numbers
            $conn->query("SET @count = 0");
            $conn->query("UPDATE class_4 SET Roll_no = @count:= @count + 1");
            $conn->query("ALTER TABLE class_4 AUTO_INCREMENT = 1");

            // Perform JavaScript redirection
            echo "<script>alert('Student record deleted successfully.');
                  window.location.href = 'class4.php';</script>";
            exit();
        } else {
...
<body>
    <div class="navbar">
        ...
        <a href="add_student4.php">Add New Student</a>
    ...
    <div class="container">
        ...
        <div class="table-container">
            <table border="1">
                ...
                $sql = "SELECT Name, Roll_no, Address, Father_name, Contact_no, Total_fees, Cleared_Fees, (Total_fees - Cleared_Fees) AS Remaining_Fees FROM class_4";
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>
                                <form method='GET' action='update_form4.php' style='display:inline;'>
