...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete'])) {
    if (isset($_POST['roll_no'])) {
        ...
        // Prepare the SQL statement
         $stmt = $conn->prepare("DELETE FROM class_10 WHERE Roll_no = ?");
        $stmt->bind_param("i", $roll_no);

        if ($stmt->execute() === TRUE) {
            // Reorder the roll numbers
            $conn->query("SET @count = 0");
            $conn->query("UPDATE class_10 SET Roll_no = @count:= @count + 1");
            $conn->query("ALTER TABLE class_10 AUTO_INCREMENT = 1");

            echo "<script>alert('Student record deleted and roll numbers updated.');
                  window.location.href = 'class10.php';</script>";
            exit();
        } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add'])) {
    ...
    function generate_new_roll_number($conn) {
        ...
        $stmt_check = $conn->prepare("SELECT Roll_no FROM class_10 WHERE Roll_no = ?");
    ...
    $stmt_insert = $conn->prepare("INSERT INTO class_10 (Name, Roll_no, Address, Father_name, Contact_no, Total_fees, Cleared_Fees)
                                    VALUES (?, ?, ?, ?, ?, ?, ?)");
...
<body>
  <div class="navbar">
        ...
        <a href="add_student10.php">Add New Student</a>
    ...
    <div class="container">
        ...
        <div class="table-container">
            <table border="1">
                ...
                $sql = "SELECT Name, Roll_no, Address, Father_name, Contact_no, Total_fees, Cleared_Fees, (Total_fees - Cleared_Fees) AS Remaining_Fees FROM class_10";
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>
                                <form method='GET' action='update_form10.php' style='display:inline;'>
